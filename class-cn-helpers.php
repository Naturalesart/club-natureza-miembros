...
class CN_Helpers {
	...
	/**
	 * Normaliza un celular argentino a un formato canónico (solo dígitos,
	 * sin +54 / 54 / 0 / 15). Usar SIEMPRE esta función en alta, login y suscripción.
	 *
...
if ( $es_argentina && substr( $digits, 0, 1 ) === '9' ) {
	$digits = substr( $digits, 1 );
		...
		if ( substr( $digits, 0, 1 ) === '9' ) {if ( $es_argentina && substr( $digits, 0, 1 ) === '9' ) {}			$digits = substr( $digits, 1 );
		}
	...
	if ( $es_argentina ) {
		...
		// Sacar el "9" de mvil que suele acompaar al cdigo de pas en formato internacional.
		...
		// Sacar "15" despus del cdigo de rea (formato local: AREA + 15 + NUMERO).
		// Los cdigos de rea argentinos van de 2 a 4 dgitos, probamos en ese orden.
		foreach ( array( 2, 3, 4 ) as $pos ) {
			if ( substr( $digits, $pos, 2 ) === '15' ) {
				$digits = substr( $digits, 0, $pos ) . substr( $digits, $pos + 2 );
				break;
		...
		// Un celular argentino normalizado (cdigo de rea + nmero, sin 9/0/15/54) tiene 10 dgitos.
		$valido = ( strlen( $digits ) === 10 && ctype_digit( $digits ) );
	} else {
		// Otros pases: se guarda tal cual aparece en WhatsApp, sin el "+".
		$valido = ( strlen( $digits ) >= 8 && strlen( $digits ) <= 15 && ctype_digit( $digits ) );
	}
	...
	public static function extraer_youtube_id( $url ) {
		...
		// Si ya nos pasaron directamente el ID.
